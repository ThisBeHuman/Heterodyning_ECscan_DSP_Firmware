...
*              ADSP-21469/79/89 EZ-Board containing the STMicro M29W320EB flash  *
*              device.                                                           *
*                                                                                *
**********************************************************************************/
	...
	#include <cdef21469.h>
	#include <def21469.h>
	static char *pEzKitTitle = "ADSP-21469 EZ-Board";
#elif __ADSP21479__
	#include <cdef21479.h>
	#include <def21479.h>
...
#include <drivers\flash\m29w320.h>
#include <sru.h>
#include <sysreg.h>
...
void Init_PLL(void)
{
	...
	/* CLKIN= 25 MHz, Multiplier= 18, Divisor= 2, CCLK_DDR2CLK_RATIO 2
	   Fcclk = (CLKIN * 2 * M) / (N * D)
	   VCO max frequency = 1000 GHz
	   M = 1 to 64, N = 2, 4, 8, 16 and D = 1 if INDIV = 0, D = 2 if INDIV = 1 */

	/* Core clock = (25MHz * 2 * 18) / (2 * 1) = 450 MHz
	   DDR2 clock = 450 / 2 = 225 MHz */

#ifdef __ADSP21469__	/* the original code */
	pmctl = ( PLLM18 | DDR2CKR2 | LPCKR3 | DIVEN );
#elif (defined(__ADSP21479__) || defined(__ADSP21489__) )
	pmctl = ( PLLM18 | DIVEN );
#endif
	*pPMCTL = pmctl;

	pmctl |= PLLBP;			/* place pll in bypass */
	pmctl &= ~DIVEN;		/* clear DIVEN while pll is in bypass */
	*pPMCTL = pmctl;
    ...
    *pPMCTL &= ~PLLBP;	/* take pll out of bypass */
...
ERROR_CODE ProcessCommand()
{
	...
	switch ( AFP_Command )
	{
		...
		case FLASH_ERASE_ALL:
			...
			ErrorCode = m29w320_Control( CNTRL_ERASE_ALL, &CmdStruct );
			break;
		...
		case FLASH_ERASE_SECT:
			...
			ErrorCode = m29w320_Control( CNTRL_ERASE_SECT, &CmdStruct);
			break;
		...
		case FLASH_GET_CODES:
			...
			ErrorCode = m29w320_Control( CNTRL_GET_CODES, &CmdStruct);
			break;
		...
		case FLASH_GET_SECTNUM:
			...
			ErrorCode = m29w320_Control( CNTRL_GET_SECTNUM, &CmdStruct);
			break;
		...
		case FLASH_GET_SECSTARTEND:
			...
			ErrorCode = m29w320_Control( CNTRL_GET_SECSTARTEND, &CmdStruct );
			break;
		...
		case FLASH_RESET:
			...
			ErrorCode = m29w320_Control( CNTRL_RESET, &CmdStruct);
			break;
...
ERROR_CODE GetNumSectors(void)
{
	...
	ErrorCode = m29w320_Control( CNTRL_GETNUM_SECTORS, (COMMAND_STRUCT *)&SGetNumSectors  );
...
ERROR_CODE GetSectorMap(SECTORLOCATION *pSectInfo)
{
	...
	for( i=0;i<AFP_NumSectors; i++)
	{
		...
		ErrorCode = m29w320_Control( CNTRL_GET_SECSTARTEND, (COMMAND_STRUCT *)&SSectStartEnd  );
	}
...
ERROR_CODE GetFlashInfo()
{
	...
	if(!ErrorCode)
	{
		ErrorCode = m29w320_Control( CNTRL_GET_DESC, &CmdStruct );
	...
	if( AFP_Verify == TRUE )
	{
		...
		for (i = 0; ( ( i < lCount ) && ( ErrorCode == NO_ERR ) ); i++, ulStartAddr += ( lStride ) )
		{
			...
			if( NO_ERR == ErrorCode )
			{
				...
				ErrorCode = m29w320_Write( (unsigned short*)&pnData[0], ulStartAddr, 0x1 );
				ErrorCode = m29w320_Read( (unsigned short*)&nCompare, ulStartAddr, 0x1 );
			...
			if( NO_ERR == ErrorCode )
		 	{
				...
				ErrorCode = m29w320_Write( (unsigned short*)&pnData[i], ulAbsoluteAddr, 0x1 );

				ErrorCode = m29w320_Read( (unsigned short*)&nCompare, ulAbsoluteAddr, 0x1 );
...
ERROR_CODE ReadData( unsigned long ulStart, long lCount, long lStride, int *pnData )
{
	...
	for (i = 0; (i < lCount) && (i < BUFFER_SIZE); i++, ulAbsoluteAddr += lStride)
	{
		...
		if( NO_ERR == ErrorCode )
		{
			ErrorCode = m29w320_Read( (unsigned short*)&pnData[i], ulAbsoluteAddr, 0x1 );
		}
		else
		{
			return ErrorCode;
